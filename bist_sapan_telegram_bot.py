...
# GÜNCELLEME 13.08.2026 — 40'lık ATR×R:R ızgarası (2022–Haz 2026, 77.879 işlem)
#   Eski: ATR 1.5 / R:R 1.5 -> PF 1.22, maliyetli reel getiri NEGATİF
#   Yeni: ATR 2.0 / R:R 2.5 -> PF 1.53, slipaj toleransı 2.5x
PORTFOY        = 950_000
RISK_YUZDESI   = 1.0
EMA_TOLERANS   = 0.02   # %2 — DEĞİŞMEDİ (ızgarada test edilmedi, sabit tutuluyor)
ATR_KATSAYI    = 2.0    # Stop = Giriş − ATR × 2.0   (eski: 1.5)
RR_KATSAYI     = 2.5    # Hedef = Giriş + 1R × 2.5   (eski: 1.5)
ZAMAN_STOPU    = 30     # gün — DEĞİŞMEDİ (asıl kâr motoru, çıkışların ~%50'si)
ENDEKS_SEMBOL  = "XU100.IS"

# ─── FİLTRE ───────────────────────────────────────────────────────────────────
# TOP50 (★) alt kümesi 4 yılda PF 1.86, dışı 1.09. True yapılırsa yalnız ★ gönderilir.
SADECE_TOP50   = False  # önce iki grubu da izle, karşılaştır, sonra karar ver
...
def main():
    ...
    # Opsiyonel TOP50 filtresi
    if SADECE_TOP50:
        elenen  = len([s for s in sinyaller if not s["top50"]])
        sinyaller = [s for s in sinyaller if s["top50"]]
        if elenen:
            print("TOP50 filtresi:", elenen, "sinyal elendi.")
