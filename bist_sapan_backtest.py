...
st.set_page_config(
    page_title="BIST Sapan Backtest",
    page_icon="📊",
    layout="wide",
...
# ─── SAPAN SİNYAL TESPİTİ ────────────────────────────────────────────────────
def sapan_sinyal_uret(df, sembol, ema_tolerans, zaman_stopu):
    """
    Her gün için Sapan Stratejisi sinyali üretir.
    Giriş = onay mumunun kapanışı (bir sonraki gün açılışı yerine)
    Stop  = teyit mumu low * 0.998
    Hedef = giriş + 2.5R (veya 2R yakın direnç varsa)
    """
    sinyaller = []

    for i in range(2, len(df)):
        son        = df.iloc[i]      # onay mumu (bugün)
        onceki     = df.iloc[i-1]    # dönüş mumu
        iki_onceki = df.iloc[i-2]    # bir önceki
        ...
        # ── GİRİŞ / STOP / HEDEF ─────────────────────────────────────────────
        giris  = float(son["Close"])          # onay mumunun kapanışında giriş
        stop   = float(son["Low"]) * 0.998    # teyit mumu low - %0.2
        stop2  = float(onceki["Low"]) * 0.998 # dönüş mumu low - %0.2
...
# ─── BAŞLIK ───────────────────────────────────────────────────────────────────
st.title("📊 BIST Sapan Stratejisi Backtest")
st.caption(f"Pozisyon Yönetimli | Sapan Stratejisi | Max {max_pozisyon} Pozisyon | Zaman Stopu: {zaman_stopu} gün")

st.info(f"""
**Backtest Kuralları (Sapan Stratejisi):**
- Aynı anda maksimum **{max_pozisyon}** pozisyon açık olabilir
- Her pozisyon güncel portföyün **%{poz_yuzde:.0f}'i** kadar (~{portfoy*poz_yuzde/100:,.0f} TL)
- Giriş: Onay mumunun kapanış fiyatı
- Stop: Teyit mumu low'u × 0.998
- Hedef: Giriş + 2.5R
- Zaman stopu: **{zaman_stopu} gün** — sonuç alınamazsa kapanır
...
if st.button("🚀 Backtest Çalıştır", use_container_width=True, type="primary"):
    ...
    with st.spinner("Sinyaller hesaplanıyor..."):
        ...
        for sembol, df in hisse_verileri.items():
            sinyaller = sapan_sinyal_uret(df, sembol, ema_tolerans, zaman_stopu)
            for s in sinyaller:
                tarih = s["tarih"]
...
if "kapali" in st.session_state:
    ...
    with tab3:
        ...
        st.download_button(
            ...
            file_name=f"sapan_backtest_{datetime.now().strftime('%Y%m%d')}.csv",
            mime="text/csv"
        )
